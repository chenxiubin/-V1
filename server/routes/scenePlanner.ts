...
router.post('/scene-recipe', async (req: Request, res: Response) => {
  try {
    ...
    if (productAssetId !== checkProfile.data.productAssetId) {
      return res.status(400).json({
    ...
    if (guidedQuestions.length < 2 || guidedQuestions.length > 5) {
      return res.status(400).json({
    ...
    if (guidedAnswers.length !== guidedQuestions.length) {
      return res.status(400).json({
    ...
    for (const ans of guidedAnswers) {
      ...
      const matchingQuestion = guidedQuestions.find(q => q.id === ans.questionId);
    ...
    for (const d of sceneDirections) {
    ...
    if (!directionIds.has(selectedDirectionId)) {
      return res.status(400).json({
