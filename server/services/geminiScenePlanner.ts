...
export class GeminiScenePlannerService {
  ...
  async analyzeSceneMatch(input: {
    ...
    const recipeJson = JSON.stringify(input.recipe, null, 2);
    const promptDocJson = JSON.stringify(input.promptDocument, null, 2);
    const productName = input.productAsset.name;

    const promptText = `You are an expert AI Scene Match Analyzer for product commercial photography.
...
Please perform a strict matching analysis across four key categories:
...
Output a strict JSON matching this structure:
{
...
`;
    ...
    // Enforce matching ids
    parsed.id = `match-report-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    parsed.recipeId = input.recipe.recipeId;
    parsed.sourceImageId = input.sceneImage.id;
